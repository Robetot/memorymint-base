 ...
 * 
 * FIXES APPLIED:
 * ✅ FIX #1: receive() REMOVED - prevents mixing mint fees with bonus pool
 * ✅ FIX #2: Per-token ERC721Receiver check in batchMint
 * ✅ FIX #3: All state variables explicitly declared
 * ✅ FIX #4: All custom errors declared
 * ✅ FIX #5: All events declared
 * ✅ FIX #6: Constants added (CURRENCY_ETH, CURRENCY_USDC, MAX_BATCH_SIZE)
 * ✅ FIX #7: BASE_USDC address hardcoded (cannot override)
 * ✅ FIX #8: MintFeesWithdrawn event added
 * ✅ FIX #9: getAvailableFees() view function added
 * 
 * FEATURES:
 ...
 * ✅ Bonus pools with deposit/withdraw (ETH + USDC) - ISOLATED from fees
 ...
 */
contract MemoryMintUltraV2Fixed is IERC721, IERC721Metadata, IERC165 {
    
    // ═══════════════════════════════════════════════════════════════════════
    //                              CONSTANTS
    // ═══════════════════════════════════════════════════════════════════════
    
    /// @notice Base Mainnet USDC contract address (HARDCODED - cannot override)
    ...
    /// @dev Minting paused state
    bool public paused;
    
    /// @dev Per-block throttle enabled
    ...
    /// @notice ETH bonus pool balance (ISOLATED from mint fees)
    uint256 public bonusPoolETH;
    
    /// @notice USDC bonus pool balance (ISOLATED from mint fees)
    ...
    /// @dev FIX: Mint fees stay in contract balance (NOT added to bonus pool)
     ...
     * @dev FIX: Per-token ERC721Receiver check for contract recipients
    ...
    function batchMint(uint256 quantity) external payable whenNotPaused whenNotKilled nonReentrant returns (uint256 startTokenId) {
        ...
        for (uint256 i = 0; i < quantity;) {
            ...
            // FIX: Per-token ERC721Receiver check for contract recipients
            if (isContract) {
     ...
     * @dev FIX: Properly calculates available fees excluding bonus pools
     */
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //                     FIX: NO receive() FUNCTION
    // ═══════════════════════════════════════════════════════════════════════
    // 
    // IMPORTANT: receive() has been intentionally REMOVED.
    // 
    // Reason: In the previous version, receive() added incoming ETH to 
    // bonusPoolETH, which mixed mint fees with bonus pool funds.
    // 
    // Solution: Use depositETH() explicitly to fund the bonus pool.
    // Mint fees (paid via payable mint functions) stay as contract balance
    // and can be withdrawn separately via withdrawMintFees().
    //
    // ═══════════════════════════════════════════════════════════════════════
}
